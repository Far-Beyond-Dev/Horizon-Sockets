...
impl Runtime {
    ...
    pub fn run_with_timeout<F: FnMut(&mio::event::Event)>(&mut self, timeout: Duration, mut f: F) -> io::Result<()> {
        loop {
    ...
    pub fn register_udp(&self, socket: &mut MioUdpSocket, token: Token, interest: Interest) -> io::Result<NetHandle> { 
        self.poll.registry().register(socket, token, interest)?;
        Ok(NetHandle)
    }
    
    /// Registers a TCP listener for connection events
    pub fn register_tcp_listener(&self, listener: &mut MioTcpListener, token: Token) -> io::Result<NetHandle> { 
        self.poll.registry().register(listener, token, Interest::READABLE)?;
        Ok(NetHandle)
    }
    
    /// Registers a TCP stream for I/O events
    pub fn register_tcp_stream(&self, stream: &mut MioTcpStream, token: Token, interest: Interest) -> io::Result<NetHandle> { 
